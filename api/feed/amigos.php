...
/**
 * ======================================================
 * CAMINHO: /home/elab/app.elab.social/api/feed/amigos.php
 * NOME: API Feed Comunidade V2 — Amigos
 *
 * DESCRIÇÃO:
 * - Retorna feed da rede próxima da pessoa logada
 * - Inclui:
 *   - eventos da própria pessoa
 *   - eventos de indicados/rede via rede_indicacoes
 *   - posts oficiais/metaverso
...
try {
    ...
    $params = [
        ...
        ':pessoa_id_c' => $pessoaId,
        ':pessoa_id_d' => $pessoaId,
        ':pessoa_id_e' => $pessoaId,
    ];
...
$params[':cursor_origem_id'] = $cursorOrigemId;
    ...
    $sql = "
        SELECT
            ...
            CASE
                WHEN f.pessoa_id = :pessoa_id_a THEN 'voce'
                WHEN ri.indicado_id IS NOT NULL THEN 'rede'
                WHEN f.grupo = 'metaverso' THEN 'oficial'
                ELSE 'outro'
            END AS relacao_feed

        FROM vw_comunidade_feed_unificado f

        LEFT JOIN rede_indicacoes ri
            ON ri.indicador_id = :pessoa_id_b
           AND ri.indicado_id = f.pessoa_id

        LEFT JOIN comunidade_visualizacoes cv
            ON cv.tenant_cliente_id = :tenant_cliente_id_a
           AND cv.pessoa_id = :pessoa_id_c
        ...
        WHERE f.tenant_cliente_id IN (0, 1, :tenant_cliente_id_e)
          AND (
                f.pessoa_id = :pessoa_id_e
                OR ri.indicado_id IS NOT NULL
                OR f.grupo = 'metaverso'
          )
    ...
    foreach ($rows as $row) {
        $item = feedNormalizarItem($row);
        $item['relacao_feed'] = (string) ($row['relacao_feed'] ?? 'outro');
        $items[] = $item;
    }
    ...
    feedLogErro('amigos', $e);

    feedJson([
        'ok' => false,
        'erro' => 'erro_feed_amigos',
        'mensagem' => 'Não foi possível carregar o feed de amigos agora.',
    ], 500);
}