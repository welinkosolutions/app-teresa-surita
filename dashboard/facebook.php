...
$stmt = $pdo->prepare("
    ...
    LEFT JOIN social_posts_cliques_app sc
        ...
        AND sc.pessoa_id = ?
    WHERE sp.ativo = 'sim'
      AND sp.rede IN ('facebook','multiplas')
      AND (
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="mb-0">Postagens no Facebook</h5>
...
<div class="card card-post mb-4 shadow-sm <?= $post['ja_clicado'] ? 'post-visto' : '' ?>">
    ...
    <div class="card-body">
        ...
        <?php if (!$post['ja_clicado']): ?>
            ...
            <button
                onclick="abrirFacebook(this, <?= (int)$post['id'] ?>, '<?= htmlspecialchars($post['link_facebook'], ENT_QUOTES) ?>', <?= (int)$post['pontos_base'] ?>)"
                class="btn btn-primary w-100">
                Ver no Facebook e ganhar pontos
            </button>
...
<script>
function abrirFacebook(botao, postId, link, pontos) {

    botao.disabled = true;
    botao.innerText = 'Processando...';

    fetch('facebook-click.php', {
        method: 'POST',
    ...
    .then(resp => {
        ...
        if (resp.status === 'ok' || resp.status === 'ja_pontuado') {
            ...
            botao.innerText = 'Post já visualizado';
            botao.disabled = true;
