...
$homeCleanNetworkRaw = strtolower((string) ($missaoCard['network'] ?? $payloadMissao['network'] ?? 'instagram'));

$homeCleanRedeLabel = $homeCleanNetworkRaw === 'facebook' ? 'Facebook' : 'Instagram';
$homeCleanRedeIcon = $homeCleanNetworkRaw === 'facebook'
    ? '/assets/feed/statics/002-facebook.svg'
    : '/assets/feed/statics/001-instagram.svg';

$homeCleanAction = strtolower(trim((string) ($missaoCard['tipo_acao'] ?? $missaoCard['acao'] ?? 'comentar')));

$homeCleanActionText = 'comente';
if ($homeCleanAction === 'curtir') {
    $homeCleanActionText = 'curta';
} elseif ($homeCleanAction === 'compartilhar') {
    $homeCleanActionText = 'compartilhe';
}

$homeCleanPontos = (int) ($missaoCard['pontos'] ?? $payloadMissao['pontos'] ?? 25);
if ($homeCleanPontos <= 0) {
    $homeCleanPontos = 25;
}

$homeCleanNivelNome = (string) (
    $nivelAtualNome
    ?? $nivelNome
    ?? $perfilUsuario['nivel_nome']
    ?? $pessoa['nivel_nome']
...
$homeCleanNivelNumero = (int) (
    $nivelAtual
    ?? $nivelAtualEstado
    ?? $perfilUsuario['nivel_atual']
    ?? $pessoa['nivel_atual']
    ?? 1
);

$homeCleanXpAtual = (int) (
    $xpAtual
    ?? $usuarioXp
    ?? $gamificacaoXp
    ?? $xpTotal
...
if ($homeCleanXpProximo <= 0) {
    $homeCleanXpProximo = 10;
}

$homeCleanXpPercent = max(3, min(100, (int) (
    $percentualNivel
    ?? round(($homeCleanXpAtual / max(1, $homeCleanXpProximo)) * 100)
)));

$homeCleanNivelIcone = '/assets/statics/awards/001-startup.svg';

$homeCleanNivelIcones = [
...
if (isset($homeCleanNivelIcones[$homeCleanNivelNumero])) {
    $homeCleanNivelIcone = $homeCleanNivelIcones[$homeCleanNivelNumero];
}

$homeCleanCaption = trim((string) ($missaoCard['caption'] ?? $payloadMissao['caption'] ?? $missaoCard['descricao'] ?? ''));
if ($homeCleanCaption === '') {
    $homeCleanCaption = 'Post selecionado para sua missão de hoje.';
}
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* =========================================================
           HOME CLEAN V1 — tela nova baseada no Perfil
        ========================================================= */
        ...
        /* Esconde header/card antigos como composição visual */
        .home-v2-header,
        .home-v2-mission-card,
        .home-v2-player-strip,
        .home-v2-advance-card {
        ...
        .home-clean-hero {
            ...
            overflow: hidden;
            background:
                radial-gradient(circle at 92% 24%, rgba(255,255,255,.16) 0 54px, transparent 55px),
                radial-gradient(circle at 7% 84%, rgba(255,255,255,.13) 0 76px, transparent 77px),
                linear-gradient(135deg, #ffbf3f 0%, #fb8c19 52%, #f97316 100%);
        ...
        .home-clean-name h1 {
            ...
            letter-spacing: -.07em;
            font-weight: 1000;
            text-shadow: 0 4px 16px rgba(124, 45, 18, .15);
        }

        .home-clean-level-card {
            position: relative;
            z-index: 2;
            margin-top: 18px;
            padding: 13px;
            ...
            box-shadow: 0 18px 34px rgba(124,45,18,.13);
            backdrop-filter: blur(10px);
            -webkit-backdrop-filter: blur(10px);
        }

        .home-clean-level-top {
            display: grid;
        ...
        .home-clean-level-number strong {
            color: #0f172a;
            font-size: 26px;
            line-height: .85;
            font-weight: 1000;
        ...
        .home-clean-instruction p strong {
            color: #16a34a;
        }
        ...
        .home-clean-points-icon {
            width: 54px;
            height: 54px;
            border-radius: 18px;
            background: radial-gradient(circle at 30% 20%, #fff 0%, #fef3c7 30%, #facc15 100%);
            display: grid;
            place-items: center;
            color: #92400e;
            font-size: 24px;
        ...
        .home-clean-mission:active {
            transform: scale(.985);
        }

        .home-clean-mission.is-clean-clicked .home-clean-points-icon {
            animation: cleanPop .65s ease both;
        }
