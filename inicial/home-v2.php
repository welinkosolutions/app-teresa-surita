...
$homeStartNivelNumero = (int) ($nivelAtual ?? 1);
$homeStartNivelNome = (string) ($nivelNome ?? 'Iniciante');
$homeStartXpAtual = (int) ($xpTotal ?? 0);

$homeStartXpProximo = match (true) {
    $homeStartNivelNumero <= 1 => 10,
    $homeStartNivelNumero === 2 => 30,
    $homeStartNivelNumero === 3 => 60,
    $homeStartNivelNumero === 4 => 100,
    $homeStartNivelNumero === 5 => 160,
    default => max(200, $homeStartXpAtual + 50),
};

$homeStartXpPercent = max(3, min(100, (int) round(($homeStartXpAtual / max(1, $homeStartXpProximo)) * 100)));
...
// Header home: normaliza XP/faltante para evitar aviso e meta menor que XP atual.
$homeStartNivelNumero = (int) ($homeStartNivelNumero ?? $nivelAtual ?? 1);
$homeStartNivelNome = (string) ($homeStartNivelNome ?? $nivelNome ?? 'Iniciante');
$homeStartXpAtual = (int) ($homeStartXpAtual ?? $xpTotal ?? 0);

$homeStartXpProximo = (int) ($homeStartXpProximo ?? 0);

if ($homeStartXpProximo <= $homeStartXpAtual) {
    $homeStartXpProximo = match (true) {
...
$homeStartXpPercent = max(
    4,
    min(100, (int) round(($homeStartXpAtual / max(1, $homeStartXpProximo)) * 100))
);

$homeStartNivelIcones = $homeStartNivelIcones ?? [
    1 => '/assets/statics/awards/001-startup.svg',
    2 => '/assets/statics/awards/007-coin.svg',
    3 => '/assets/statics/awards/008-shield.svg',
    4 => '/assets/statics/awards/006-energy-bar.svg',
    ...
    <style>
        ...
        * {
        ...
        .home-start-header {
            ...
            overflow: hidden;
            background:
                radial-gradient(circle at 92% 26%, rgba(255,255,255,.18) 0 54px, transparent 55px),
                radial-gradient(circle at 7% 88%, rgba(255,255,255,.13) 0 80px, transparent 81px),
                linear-gradient(135deg, #ffbf3f 0%, #fb8c19 52%, #f97316 100%);
            box-shadow: 0 18px 42px rgba(249, 115, 22, .22);
        }

        .home-start-profile-pill {
            display: inline-flex;
            align-items: center;
            justify-content: center;
            height: 26px;
        ...
        .home-start-level-inline img {
            width: 18px;
            height: 18px;
            object-fit: contain;
        }

        .home-start-level-inline span {
            font-size: 10px;
            font-weight: 1000;
            white-space: nowrap;
        }

        .home-start-level-inline strong {
            color: #16a34a;
            font-size: 11px;
            line-height: 1;
            font-weight: 1000;
        ...
        .home-start-level-badge {
            ...
            display: grid;
            place-items: center;
            box-shadow: 0 14px 26px rgba(124,45,18,.13);
        }

        .home-start-level-badge span {
            display: block;
            color: #92400e;
            font-size: 9px;
            line-height: 1;
        ...
        .home-start-progress-copy strong {
            color: #64748b;
        }
        ...
        <div class="home-start-top">
            <div class="home-start-user">
                <div class="home-start-avatar">
                    <img src="<?= home_h($avatarUsuario) ?>" alt="">
                </div>
